...
export default function Projects() {
  ...
  let acount = useRecoilValue<string | null>(AccountAtom);
  let context = useRecoilValue(ContextAtom)
  let [projectArr, setProjectArr] = useState<IProject[] | null>(null);
  const [counter, setCounter] = useState(0)
  useEffect(() => {
    if(projectArr && projectArr.length > 0) return
    let task = async () => {
      let projects: IProject[] = [];
      let resp = await getProject()
      if(resp.ok){
        let strapiData = await resp.json()
        projects = strapiData.data.map((v: any) => ({...v.attributes}))
      }
